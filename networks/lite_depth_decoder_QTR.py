...
class Lite_Depth_Decoder_QueryTr(nn.Module):
    def __init__(self, in_channels, embedding_dim=128, patch_size=16, num_heads=4, query_nums=100, dim_out=256, norm='linear',
        ...
        self.positional_encodings = nn.Parameter(torch.rand(1000, embedding_dim), requires_grad=True)
    ...
    def forward(self, x0):
        ...
        regression_head, queries = total_queries[0, ...], total_queries[1:self.query_nums + 1, ...]
        queries = queries.permute(1, 0, 2)

        energy_maps, _ = self.full_query_layer(x0, queries)
